...
import com.example.yasmine.inventoryapp.data.InventoryContract.ProductEntry;
...
public class InventoryProvider extends ContentProvider {
    ...
    private Uri insertProduct(Uri uri, ContentValues values) {
        ...
        Integer available = values.getAsInteger(ProductEntry.COLUMN_PRODUCT_AVAILABLE);
        if (available == null || !ProductEntry.isAvailable(available)) {
            throw new IllegalArgumentException("Product requires valid availablity");
        }
    ...
    private int updateProduct(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        // If the {@link PetEntry#COLUMN_PRODUCT_AVAILABLE} key is present,
        // check that the product value is valid.
        if (values.containsKey(ProductEntry.COLUMN_PRODUCT_AVAILABLE)) {
            Integer available = values.getAsInteger(ProductEntry.COLUMN_PRODUCT_AVAILABLE);
            if (available == null || !ProductEntry.isAvailable(available)) {
                throw new IllegalArgumentException("Product requires valid availablity");
            }
